    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const savedHotelData: HotelInventory = {
                ...
                createdAt: responseData.data?.createdAt?.split('T')[0] || hotel?.createdAt || formatDateToYYYYMMDD(new Date()),
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className={`w-full px-5 py-4 text-base border-2 rounded-xl transition-all duration-200 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-4 focus:outline-none shadow-sm hover:shadow-md font-semibold ${!formData.location.country
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                            {availableAmenitiesList.map((amenity) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`p-2 rounded-lg mr-3 ${isSelected ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'
                                            }`}>
                                            <IconComponent className={`w-5 h-5 ${isSelected ? 'text-white' : 'text-gray-600 dark:text-gray-400'}`} />
                                        </div>
                                        <div className="text-left">
                                            <p className={`font-semibold text-sm ${isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300'
                                                }`}>
                                                {amenity.name}
                                            </p>
                                        </div>
                    ...
                    <div className="bg-gradient-to-br from-orange-50 via-white to-orange-50/30 dark:from-gray-800 dark:via-gray-800 dark:to-gray-800 p-8 rounded-2xl border-2 border-orange-100 dark:border-gray-700 shadow-lg">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <div className="relative">
                                    <DatePicker
                                        selected={parseYYYYMMDD(formData.checkInDate)}
                                        onChange={(date) => {
                                            const dateString = formatDateToYYYYMMDD(date);
                                            setFormData(prev => ({ ...prev, checkInDate: dateString, checkOutDate: '' })); // Reset checkout date
                                        }}
                                        minDate={new Date()}
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <DatePicker
                                        selected={parseYYYYMMDD(formData.checkOutDate)}
                                        onChange={(date) => {
                                            const dateString = formatDateToYYYYMMDD(date);
                                            setFormData(prev => ({ ...prev, checkOutDate: dateString }));
                                        }}
                                        minDate={parseYYYYMMDD(formData.checkInDate) || new Date()}
                                        disabled={!formData.checkInDate}
                                        dateFormat="MMMM d, yyyy"
                                        placeholderText="Select check-out date"
                                        className="w-full px-4 py-3 bg-white dark:bg-gray-700 border rounded-xl focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all text-base cursor-pointer border-gray-300 dark:border-gray-600 disabled:bg-gray-100"
                                        wrapperClassName="w-full"
                                        showPopperArrow={false}
                                        showMonthDropdown
                                        showYearDropdown
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        if (formData.checkInDate && formData.checkOutDate) {
                                            const checkIn = parseYYYYMMDD(formData.checkInDate);
                                            const checkOut = parseYYYYMMDD(formData.checkOutDate);
                                                ...
                                                <div className="flex items-center space-x-4">
                                                    <div>
                                                        ...
                                                        <p className="text-base font-bold text-gray-900 dark:text-white">
                                                            {parseYYYYMMDD(dateItem.checkIn)?.toLocaleDateString('en-US', {
                                                                weekday: 'short',
                                                                year: 'numeric',
                                                                month: 'short',
                                                                day: 'numeric'
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-base font-bold text-gray-900 dark:text-white">
                                                            {parseYYYYMMDD(dateItem.checkOut)?.toLocaleDateString('en-US', {
                                                                weekday: 'short',
                                                                year: 'numeric',
                                                                month: 'short',
                                                                day: 'numeric'
