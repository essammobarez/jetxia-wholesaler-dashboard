            ...
            <div className="space-y-4">
                <div>
                    ...
                    <div className="relative">
                        ...
                        <DatePicker
                            selected={parseYYYYMMDD(formData.dates.startDate)}
                            onChange={(date: Date | null) => {
                                setFormData(prev => ({
                                    ...
                                    dates: { 
                                        ...
                                        endDate: '' // Reset end date if start date changes
                                    }
                                }));
                            ...
                            className={`w-full pl-12 pr-4 py-3.5 text-base font-semibold bg-white dark:bg-gray-800 border-2 rounded-xl focus:ring-4 focus:ring-red-200 dark:focus:ring-red-900/30 shadow-sm hover:shadow-md cursor-pointer transition-all ${errors.startDate
                ...
                <div>
                    ...
                    <div className="relative">
                        <Calendar className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-red-600 dark:text-red-400 pointer-events-none z-10" />
                        <DatePicker
                            selected={parseYYYYMMDD(formData.dates.endDate)}
                            onChange={(date: Date | null) => {
                                setFormData(prev => ({
                                    ...prev,
                                    dates: { ...prev.dates, endDate: formatDateToYYYYMMDD(date) }
                                }));
                                setErrors(prev => ({ ...prev, endDate: '' }));
                            }}
                            dateFormat="dd/MM/yyyy"
                            placeholderText="Select end date"
                            className={`w-full pl-12 pr-4 py-3.5 text-base font-semibold bg-white dark:bg-gray-800 border-2 rounded-xl focus:ring-4 focus:ring-red-200 dark:focus:ring-red-900/30 shadow-sm hover:shadow-md cursor-pointer transition-all ${errors.endDate
                                ? 'border-red-500 focus:border-red-500'
                                : 'border-red-300 dark:border-red-700 focus:border-red-500 dark:focus:border-red-400'
                                }`}
                            minDate={parseYYYYMMDD(formData.dates.startDate) || new Date()}
                            disabled={!formData.dates.startDate}
                            showPopperArrow={false}
                            autoComplete="off"
                        />
                    </div>
                ...
                <div>
                    ...
                    <div className="relative">
                        ...
                        <DatePicker
                            selected={parseYYYYMMDD(formData.dates.bookingDeadline)}
                            onChange={(date: Date | null) => {
                                setFormData(prev => ({
                                    ...prev,
                                    dates: { ...prev.dates, bookingDeadline: formatDateToYYYYMMDD(date) }
                                }));
                            }}
                            dateFormat="dd/MM/yyyy"
                            ...
                            minDate={new Date()}
                            maxDate={parseYYYYMMDD(formData.dates.startDate) || undefined}
                            showPopperArrow={false}
                            autoComplete="off"
                        />
                    </div>
                    ...
                    <div className="p-4 bg-gradient-to-r from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-xl border-2 border-red-200 dark:border-red-800">
                        <div className="flex items-center justify-between text-sm">
                            <div className="flex items-center text-gray-700 dark:text-gray-300">
                                ...
                                <span className="font-semibold">
                                    {parseYYYYMMDD(formData.dates.startDate)?.toLocaleDateString('en-US', {
                                        day: 'numeric',
                                        month: 'short',
                                        year: 'numeric'
                                    }) || '...'}
                            ...
                            <div className="flex items-center text-gray-700 dark:text-gray-300">
                                <span className="font-semibold">
                                    {parseYYYYMMDD(formData.dates.endDate)?.toLocaleDateString('en-US', {
                                        day: 'numeric',
                                        month: 'short',
                                        year: 'numeric'
                                    }) || '...'}
                        ...
                        <div className="mt-2 pt-2 border-t border-red-200 dark:border-red-700 text-center">
                            <p className="text-xs font-bold text-red-800 dark:text-red-300">
                                {(() => {
                                    const start = parseYYYYMMDD(formData.dates.startDate);
                                    const end = parseYYYYMMDD(formData.dates.endDate);
