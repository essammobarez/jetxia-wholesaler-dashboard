...
interface Payment {
    ...
    matchingDifference: number; // Represents the unmatched amount
    createdAt: string;
}
...
const PaymentReport = () => {
    ...
    // New states for supplier selection and payment fetching
    ...
    const [supplierBookingsLoading, setSupplierBookingsLoading] = useState(false);
    ...
    // Effect for fetching initial page data (Dashboard, Suppliers)
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (dashboardResult.success) {
                    ...
                    throw new Error(dashboardResult.message || 'Failed to retrieve dashboard data');
                }
                ...
                // Set the first supplier as the default selection
                if (allSuppliers.length > 0 && !selectedSupplierId) {
                    setSelectedSupplierId(allSuppliers[0]._id);
                }

                // NOTE: Demo bookings are kept for matching report modal functionality.
    ...
    const fetchPaymentsForSupplier = useCallback(async (supplierId: string) => {
        if (!wholesalerId || !supplierId) {
            setPayments([]);
            return;
        }
        ...
        try {
            ...
            if (result.success && Array.isArray(result.data)) {
                const selectedSupplierDetails = suppliers.find(s => s._id === supplierId);
                const fetchedPayments: Payment[] = result.data.map((p: any) => ({
                    _id: p._id,
                    paymentId: p.paymentId,
                    supplier: {
                        _id: p.supplier._id,
                        name: p.supplier.name,
                        type: selectedSupplierDetails?.type || 'offline',
                    },
                    amount: p.amount,
                    currency: p.currency,
                    ...
                    matchedBookings: p.matchedBookings?.map((b: any) => typeof b === 'object' ? b._id : b) || [],
                    totalMatchedAmount: p.matchedAmount,
                    matchingDifference: p.unmatchedAmount,
                    createdAt: p.createdAt,
                }));
                setPayments(fetchedPayments);
            } else {
                throw new Error(result.message || 'Failed to retrieve payments.');
            }
        } catch (err: any) {
    ...
    }, [wholesalerId, suppliers]);

    // Effect for fetching payments when a supplier is selected
    useEffect(() => {
        if (selectedSupplierId) {
            fetchPaymentsForSupplier(selectedSupplierId);
        }
    }, [selectedSupplierId, fetchPaymentsForSupplier]);
    ...
    const handleSaveMatching = async (paymentId: string, selectedBookingsData: BookingForMatching[]) => {
        ...
        try {
            ...
            if (!res.ok || !result.success) {
                throw new Error(result.message || 'Failed to save the match.');
            }

            // On success, close the modal and refresh the payments list.
            setMatchingModal({ payment: null, isOpen: false });
            await fetchPaymentsForSupplier(payment.supplier._id);
    ...
    const handleSupplierChange = async (supplierId: string) => {
        ...
        setPaymentForm(prev => ({ ...prev, supplierId, amount: '' }));
        setPreSelectedBookings([]); // Reset selections

        if (!supplierId || !wholesalerId) {
        ...
        try {
            ...
            const result = await res.json();
            if (result.success && result.data && Array.isArray(result.data.bookings)) {
                const selectedSupplierDetails = suppliers.find(s => s._id === supplierId);
                const apiBookings: BookingForMatching[] = result.data.bookings.map((b: any, index: number) => ({
                    _id: b.bookingId,
                    ...
                    supplier: {
                        _id: selectedSupplierDetails?._id || supplierId,
                        name: selectedSupplierDetails?.name || 'Unknown',
                        type: selectedSupplierDetails?.type || 'offline',
                    },
                    serviceDates: {
                        startDate: b.serviceDate,
                        endDate: b.serviceDate
                    },
                    status: b.status,
                    serviceType: 'Hotel',
                    ...
                    isMatched: b.fullyMatched,
                    matchedPaymentId: undefined,
                    createdAt: b.createdAt,
                    editableAmount: b.amount,
                    isPartialMatch: b.totalMatched > 0 && !b.fullyMatched,
                    remainingAmount: b.remainingAmount,
                }));

                setSupplierBookingsPreview(apiBookings);

                const totalUnmatched = apiBookings.reduce((sum, b) => sum + b.netRate, 0);
                if (totalUnmatched > 0) {
            ...
            setSupplierBookingsLoading(false);
        }
    };
    ...
    const handleCreatePayment = async () => {
        ...
        try {
            const payload = {
                ...
                supplier: paymentForm.supplierId,
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/supplier-payments/payment`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}` // --- Add token ---
            ...
            const createdPayment = result.data.payment;
            
            // After creating, refetch payments for the relevant supplier
            if (createdPayment.supplier === selectedSupplierId) {
                await fetchPaymentsForSupplier(createdPayment.supplier);
            }
            ...
            setPaymentValidation({ general: error.message || 'An unexpected error occurred. Please try again.' });
        } finally {
        ...
        <div className="space-y-4">
            ...
            <div className="card-modern p-4 bg-orange-50 dark:bg-orange-900/20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
                    <div><span className="font-medium">Suppliers:</span> {dashboardData?.suppliers ?? '...'}</div>
                    <div><span className="font-medium">Payments:</span> {dashboardData?.payments ?? '...'}</div>
                    <div><span className="font-medium">Bookings:</span> {dashboardData?.bookings ?? '...'}</div>
                    <div><span className="font-medium">Matched:</span> {dashboardData?.matched ?? '...'}</div>
                    <div><span className="font-medium">Total:</span> USD {dashboardData?.total?.toFixed(2) ?? '...'}</div>
                </div>
            </div>
            ...
            <div className="card-modern overflow-hidden">
                ...
                <div className="p-6 border-b">
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
                        <div className="bg-blue-50 p-4 rounded-lg">
                            ...
                            <p className="text-2xl font-bold text-blue-700">{dashboardData?.summary?.totalPayments?.count ?? '...'}</p>
                            {dashboardData?.summary?.totalPayments?.amount > 0 && <p className="text-sm text-blue-600">USD {dashboardData?.summary?.totalPayments?.amount?.toFixed(2) ?? '...'}</p>}
                        </div>
                        <div className="bg-green-50 p-4 rounded-lg">
                            <h3 className="text-sm font-medium text-green-600">Completed</h3>
                            <p className="text-2xl font-bold text-green-700">{dashboardData?.summary?.completed?.count ?? '...'}</p>
                            {dashboardData?.summary?.completed?.amount > 0 && <p className="text-sm text-green-600">USD {dashboardData?.summary?.completed?.amount?.toFixed(2) ?? '...'}</p>}
                        </div>
                        <div className="bg-yellow-50 p-4 rounded-lg">
                            <h3 className="text-sm font-medium text-yellow-600">Pending</h3>
                            <p className="text-2xl font-bold text-yellow-700">{dashboardData?.summary?.pending?.count ?? '...'}</p>
                            {dashboardData?.summary?.pending?.amount > 0 && <p className="text-sm text-yellow-600">USD {dashboardData?.summary?.pending?.amount?.toFixed(2) ?? '...'}</p>}
                        </div>
                        <div className="bg-purple-50 p-4 rounded-lg">
                            <h3 className="text-sm font-medium text-purple-600">Matched</h3>
                            <p className="text-2xl font-bold text-purple-700">{dashboardData?.summary?.matched?.count ?? '...'}</p>
                        </div>
                        <div className="bg-orange-50 p-4 rounded-lg">
                            <h3 className="text-sm font-medium text-orange-600">Unmatched</h3>
                            <p className="text-2xl font-bold text-orange-700">{dashboardData?.summary?.unmatched?.count ?? '...'}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 border-b flex items-center gap-4 bg-gray-50/50">
                    <label htmlFor="supplier-select" className="text-sm font-medium text-gray-700 whitespace-nowrap">
                    ...
                    <select
                        ...
                        className="w-full md:w-1/3 p-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
                        disabled={loading || suppliers.length === 0}
                    >
                        {suppliers.length === 0 && <option>Loading suppliers...</option>}
                        {suppliers.map(s => (
                ...
                <div className="overflow-x-auto">
                    {paymentsLoading ? (
                        ...
                        <div className="text-center p-12 text-red-600">Error: {paymentsError}</div>
                    ) : !selectedSupplierId ? (
                        <div className="text-center p-12 text-gray-500">Please select a supplier to view payments.</div>
                    ) : payments.length === 0 ? (
                        <div className="text-center p-12 text-gray-500">No payments found for the selected supplier.</div>
                    ) : (
                                ...
                                {payments.map((payment) => (
                                    <tr key={payment._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${payment.status === 'completed' ? 'bg-green-100 text-green-800' :
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold rounded-full ${payment.totalMatchedAmount > 0 ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'
                                                }`}>
                                                {payment.totalMatchedAmount > 0 ? (
                                                    <><CheckCircle className="w-3 h-3" /> {getCurrencySymbol(payment.currency)} {payment.totalMatchedAmount.toFixed(2)}</>
                                                ) : 'Not Matched'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-right font-mono">
                                            {payment.totalMatchedAmount > 0 ? (
                                                <span className={`font-semibold ${Math.abs(payment.matchingDifference) < 0.01 ? 'text-green-600' :
            ...
            {showCreatePaymentModal && (
                <EnhancedCreatePaymentModal
                    ...
                    isLoadingPreview={supplierBookingsLoading}
...
const PaymentMatchingModal: React.FC<{
    ...
    onClose: () => void;
    onSave: (paymentId: string, selectedBookingsData: BookingForMatching[]) => void;
    getCurrencySymbol: (code: string) => string;
}> = ({ payment, wholesalerId, onClose, onSave, getCurrencySymbol }) => {
    ...
    const [availableBookings, setAvailableBookings] = useState<BookingForMatching[]>([]);
    const [isLoadingBookings, setIsLoadingBookings] = useState(true);
    const [fetchError, setFetchError] = useState<string | null>(null);

    useEffect(() => {
        if (!payment || !wholesalerId) {
            setFetchError("Missing payment or wholesaler information.");
            ...
            try {
                ...
                if (result.success && result.data && Array.isArray(result.data.bookings)) {
                    const apiBookings: BookingForMatching[] = result.data.bookings.map((b: any, index: number) => ({
                        _id: b.bookingId,
                        bookingId: b.bookingReference,
                        agency: { agencyName: b.agencyName, _id: `agency_${index}` },
                        supplier: payment.supplier,
                        serviceDates: { startDate: b.serviceDate, endDate: b.serviceDate },
                        status: b.status,
                        serviceType: 'Hotel',
                        hotelName: b.hotelName,
                        netRate: b.amount,
                    ...
                    setAvailableBookings(apiBookings);
    ...
    const handleAmountEdit = (bookingId: string, newAmount: number) => {
        const booking = availableBookings.find(b => b._id === bookingId);
        if (!booking) return;
    ...
    const getEffectiveAmount = (bookingId: string): number => {
        const booking = availableBookings.find(b => b._id === bookingId);
        if (!booking) return 0;
        return isEditingMode ? (editableAmounts[bookingId] ?? booking.netRate) : booking.netRate;
    };
    ...
    const handleSaveClick = () => {
        const selectedBookingObjects = availableBookings
            .filter(b => selectedBookings.includes(b._id))
            .map(b => ({ ...b, editableAmount: getEffectiveAmount(b._id) }));
        onSave(payment._id, selectedBookingObjects);
    };
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className={`p-4 rounded-lg ${isOverPayment ? 'bg-red-50' : 'bg-green-50'}`}>
                            ...
                            <div className={`space-y-1 text-sm ${isOverPayment ? 'text-red-700' : 'text-green-700'}`}>
                                ...
                                <p><span className="font-medium">Difference:</span>
                                    <span className={`font-bold ml-1 ${Math.abs(payment.amount - selectedTotal) < 0.01 ? 'text-green-600' :
                        ...
                        <div className="bg-purple-50 p-4 rounded-lg">
                            ...
                            <div className="space-y-1 text-sm text-purple-700">
                                <p><span className="font-medium">Total:</span> {availableBookings.length}</p>
                                <p><span className="font-medium">Unmatched:</span> {availableBookings.filter(b => !b.isMatched).length}</p>
                                <p><span className="font-medium">Supplier Type:</span> {payment.supplier.type}</p>
                            </div>
                        </div>

                        <div className={`p-4 rounded-lg ${Math.abs(payment.amount - selectedTotal) < 0.01 ? 'bg-green-50' :
                            isOverPayment ? 'bg-red-50' : 'bg-orange-50'
                            }`}>
                            <h4 className={`font-semibold mb-2 flex items-center gap-2 ${Math.abs(payment.amount - selectedTotal) < 0.01 ? 'text-green-900' :
                            ...
                            <div className="space-y-2">
                                <p className={`font-bold text-sm ${Math.abs(payment.amount - selectedTotal) < 0.01 ? 'text-green-700' :
                                ...
                                {selectedTotal > 0 && isEditingMode && (
                                    <p className="text-xs text-purple-600">
                                        {selectedBookings.filter(id => getEffectiveAmount(id) < (availableBookings.find(b => b._id === id)?.netRate || 0)).length} partial matches
                                    </p>
                                )}
                                <button
                                    onClick={() => setIsEditingMode(!isEditingMode)}
                                    className={`w-full px-3 py-1 text-xs rounded-lg transition-colors ${isEditingMode
                                    ...
                                    <button
                                        onClick={() => {
                                            if (selectedBookings.length > 0) {
                                                ...
                                                const lastBooking = availableBookings.find(b => b._id === lastBookingId);
                        ...
                        <div className="border rounded-lg overflow-hidden max-h-96 overflow-y-auto">
                            <table className="min-w-full text-sm">
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {isLoadingBookings ? (
                                        <tr>
                                            <td colSpan={isEditingMode ? 8 : 7} className="text-center py-12 text-gray-500">
                                                Loading available bookings...
                                            </td>
                                        </tr>
                                    ) : fetchError ? (
                                        <tr>
                                            <td colSpan={isEditingMode ? 8 : 7} className="text-center py-12 text-red-600">
                                                Error: {fetchError}
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            <td colSpan={isEditingMode ? 8 : 7} className="text-center py-12">
                                                <div className="text-gray-500">
                                                    ...
                                                    <p className="text-sm">No confirmed bookings found for this supplier.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                        availableBookings.map((booking) => (
                                            <tr
                                                key={booking._id}
                                                className={`cursor-pointer transition-all ${selectedBookings.includes(booking._id) ? 'bg-orange-50 border-l-4 border-orange-500' :
                                            ...
                                            >
                                                <td className="px-4 py-3">
                                                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center ${selectedBookings.includes(booking._id) ? 'border-orange-500 bg-orange-500' :
                                                ...
                                                <td className="px-4 py-3">
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            ...
                                                            <span className={`px-2 py-0.5 text-xs rounded-full ${booking.supplier.type === 'online' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                                                }`}>
                                                    ...
                                                    <td className="px-4 py-3 text-right">
                                                        <div className="space-y-1">
                                                            <input
                                                                ...
                                                                className={`w-24 px-2 py-1 text-xs border rounded focus:ring-2 focus:ring-orange-500 text-right font-mono ${selectedBookings.includes(booking._id)
                                                                    ? 'border-orange-500 bg-orange-50'
                                                                    : 'border-gray-300'
                                                                    }`}
                                                                disabled={booking.isMatched || !selectedBookings.includes(booking._id)}
                                                            />
                                                ...
                                                <td className="px-4 py-3 text-center">
                                                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${booking.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                                                        }`}>
                                                        {booking.status}
                                                    </span>
                                                </td>
                ...
                <div className="p-6 border-t bg-gray-50 dark:bg-gray-900/50">
                    <div className="flex items-center justify-between">
                        <div className="text-sm text-gray-600 dark:text-gray-400">
                            <div className="space-y-1">
                                <p><span className="font-medium">Selected:</span> {selectedBookings.length} of {availableBookings.filter(b => !b.isMatched).length} available bookings</p>
                                ...
                                {selectedTotal > 0 && (
                                    <p>
                                        ...
                                        <span className={`font-semibold ml-1 ${Math.abs(payment.amount - selectedTotal) < 0.01 ? 'text-green-600' :
                                            isOverPayment ? 'text-red-600' : 'text-orange-600'
                                            }`}>
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`px-6 py-2 rounded-lg flex items-center gap-2 transition-colors ${isOverPayment
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        ...
                        <div className="bg-green-50 p-4 rounded-lg">
                            ...
                            <div className="space-y-1 text-sm">
                                ...
                                <p><span className="font-medium">Difference:</span>
                                    <span className={`font-bold ml-1 ${Math.abs(payment.amount - totalMatched) < 0.01 ? 'text-green-600' : 'text-orange-600'
                                        }`}>
                                    ...
                                    {bookings.map((booking) => (
                                        <tr key={booking._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2 text-center">
                                                <span className={`px-2 py-1 text-xs rounded-full ${booking.supplier.type === 'online' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                                    }`}>
                                                    {booking.supplier.type}
                                                </span>
                                            </td>
...
const EnhancedCreatePaymentModal: React.FC<{
    ...
    suppliers, paymentForm, setPaymentForm, supplierBookingsPreview, isLoadingPreview, preSelectedBookings,
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            className={`w-full px-4 py-3 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 transition-all ${validation.supplier
                                        ...
                                        <div>
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className={`w-full px-4 py-3 pr-12 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 transition-all ${validation.amount
                                                        ? 'border-red-300 dark:border-red-600'
                                                        : 'border-gray-300 dark:border-gray-600'
                                                        }`}
                                                    placeholder="0.00"
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full px-4 py-3 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 transition-all ${validation.reference
                                                ? 'border-red-300 dark:border-red-600'
                                                : 'border-gray-300 dark:border-gray-600'
                                                }`}
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            className={`w-full px-4 py-3 border rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 transition-all ${validation.description
                                                ? 'border-red-300 dark:border-red-600'
                                                : 'border-gray-300 dark:border-gray-600'
                                                }`}
                                            rows={3}
                                ...
                                <>
                                    <div>
                                        <h4 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                            ...
                                            <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs rounded-full ${selectedSupplier.type === 'online' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'
                                                }`}>
                                                <div className={`w-1.5 h-1.5 rounded-full ${selectedSupplier.type === 'online' ? 'bg-green-500' : 'bg-blue-500'
                                                    }`} />
                                        ...
                                        {supplierBookingsPreview.length > 0 ? (
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-3 gap-4">
                                                    ...
                                                    <div className={`p-3 rounded-lg text-center ${isOverPayment ? 'bg-red-50 dark:bg-red-900/20' : 'bg-green-50 dark:bg-green-900/20'
                                                        }`}>
                                                        <div className={`text-2xl font-bold ${isOverPayment ? 'text-red-700 dark:text-red-300' : 'text-green-700 dark:text-green-300'
                                                            }`}>
                                                            {getCurrencySymbol(paymentForm.currency)} {preSelectedTotal.toFixed(2)}
                                                        </div>
                                                        <div className={`text-sm ${isOverPayment ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'
                                                            }`}>
                                                        ...
                                                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                                            {supplierBookingsPreview.map((booking) => {
                                                                ...
                                                                return (
                                                                    <tr
                                                                        key={booking._id}
                                                                        className={`cursor-pointer transition-all ${preSelectedBookings.includes(booking._id)
                                                                    ...
                                                                    >
                                                                        <td className="px-3 py-2">
                                                                            <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${preSelectedBookings.includes(booking._id)
                                                                        ...
                                                                        <td className="px-3 py-2 text-center">
                                                                            <span className={`px-2 py-1 text-xs rounded-full ${booking.status === 'confirmed' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
                                                                                }`}>
                                                                                {booking.status}
                                                                            </span>
                                                                        </td>
                        ...
                        <div className="flex items-center justify-between">
                                    ...
                                    {preSelectedBookings.length > 0 && (
                                        <>
                                            ...
                                            <p>
                                                ...
                                                <span className={`ml-1 font-semibold ${Math.abs(parseFloat(paymentForm.amount || '0') - preSelectedTotal) < 0.01 ? 'text-green-600' :
                                                    isOverPayment ? 'text-red-600' : 'text-orange-600'
                                                    }`}>
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    className={`px-6 py-3 rounded-lg flex items-center gap-2 transition-colors font-medium ${isCreating
