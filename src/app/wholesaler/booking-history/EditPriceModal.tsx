...
const EditPriceModal: React.FC<EditPriceModalProps> = ({
  ...
  const calculateMarkup = useCallback(() => {
    ...
    if (
      !isNaN(newPriceValue) &&
      !isNaN(originalBasePrice) &&
      !isNaN(originalMarkup)
    ) {
      ...
      const newCalculatedMarkup =
        originalMarkup + (adjustedPrice - newPriceValue);
  ...
  useEffect(() => {
    const fetchSuppliers = async () => {
      ...
      try {
        ...
        const offlineSuppliers: Supplier[] = (
          Array.isArray(data1) ? data1 : data1.data || []
        ).map((supplier: any) => ({
          _id: supplier._id,
          name: supplier.name,
          type: "Offline",
        ...
        const onlineSuppliers: Supplier[] = (
          Array.isArray(data2) ? data2 : data2.data || []
        ).map((supplier: any) => ({
          _id: supplier._id,
          name: supplier.name,
          type: "Online",
    ...
    if (isOpen && wholesalerId) fetchSuppliers();
  }, [isOpen, wholesalerId]);
  ...
  const handleSave = async () => {
    if (!wholesalerId) {
      setSaveError("Wholesaler ID not found. Please refresh the page.");
      return;
    }

    if (!reservation.dbId) {
      setSaveError("Booking ID not found. Please refresh the page.");
      return;
    }
    ...
    const finalSellingPrice = calculateFinalSellingPrice();
    const markupValue = parseFloat(markup);

    if (isNaN(finalSellingPrice) || isNaN(markupValue)) {
      setSaveError("Invalid price or markup values.");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/booking/modify/${reservation.dbId}`,
        {
          method: "PUT",
          headers: {
      ...
      // Call the original onSave callback with the updated data
      onSave({
      ...
      console.error("Error saving price modification:", error);
      setSaveError(
  ...
  const currentReservationDetails = [
    { label: "Supplier Name", icon: FiUser, value: reservation.providerName },
    {
      label: "Supplier Rate",
      icon: FiDollarSign,
      value: reservation.priceIssueNet.toFixed(2),
    },
    ...
    { label: "Markup", icon: FiArrowUp, value: calculatedPrices.m.toFixed(2) },
    {
      label: "Commission",
      icon: FiDollarSign,
      value: calculatedPrices.c.toFixed(2),
    },
          ...
          >
            ...
            <div className="p-6 grid grid-cols-1 lg:grid-cols-5 gap-8 overflow-y-auto flex-grow">
              ...
              <div className="lg:col-span-2 bg-gray-50 dark:bg-gray-700 rounded-xl p-6 shadow-inner border border-gray-200 dark:border-gray-600">
                <h4 className="flex items-center text-lg font-bold text-gray-800 dark:text-gray-100 mb-4 pb-2 border-b border-gray-200 dark:border-gray-600">
                  <FiTag className="mr-2 text-blue-500" size={20} /> Current
                  Details
                </h4>
                <div className="space-y-4">
                  {currentReservationDetails.map(
                    ({ label, icon: Icon, value }) => (
                      <div
                        key={label}
                        className="flex items-center justify-between py-1.5 border-b border-gray-100 dark:border-gray-600 last:border-b-0"
                      >
                ...
                <div className="space-y-5">
                    ...
                    {editablePriceDetails.map(
                      ({
                        ...
                        <div key={label}>
                          ...
                          <div
                            className={`flex items-center border rounded-lg overflow-hidden transition-all duration-200 ${
                              editable
                                ? "bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div
                        className={`flex items-center border rounded-lg overflow-hidden transition-all duration-200 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:border-indigo-500`}
                      >
                        <span className="flex items-center pl-3 pr-2 text-gray-400 dark:text-gray-500">
                          <FiTag size={18} />
                        </span>
            ...
            <footer className="px-6 py-4 bg-gray-50 dark:bg-gray-800 flex justify-end space-x-3 border-t dark:border-gray-700 shrink-0">
              <button
                onClick={onClose}
                disabled={isSaving}
                className="flex items-center gap-2 px-5 py-2.5 border rounded-lg border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 text-base font-medium shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <FiX size={18} /> Cancel
              </button>
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-base font-medium shadow-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSaving ? (
                  <>
